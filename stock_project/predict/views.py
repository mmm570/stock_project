...
def newUrl(request):
    ...
    newData=[]
    h1=soup.find('h1',{'data-test-locator':"headline"})
...
def newImg(request):
    ...
    if targets in storestock:
        #f = open(r'C:/Users/sleep/Desktop/store/'+targets+'_news.txt', encoding='UTF-8') #開新聞黨
        #date1 = [] #日期
        #pre = []  #預測後解果
    ...
    fig = go.Figure()
    if targets in storestock:
        df = pd.read_csv('C:/Users/sleep/Desktop/store/'+targets+'_news.txt')
        fig.add_trace(go.Scatter(x=df.date,y=df.score))
    else:
    ...
    if targets not in storestock:
        ...
        df = pd.read_csv('C:/Users/sleep/Desktop/store/'+targets+'_price.txt')
        fig2.add_trace(go.Scatter(x=df.Date,y=df.actual_prices,name='實際股價'))
        fig2.add_trace(go.Scatter(x=df.Date,y=df.predicted_prices,name='預測股價'))
    ...
    if targets not in storestock:
        ...
        prediction=df["predicted_prices"][df.shape[0]-1]
    if targets not in storestock:
        mse=metrics.mean_squared_error(predicted_prices, test_data['Close'])
        mae=metrics.mean_absolute_error(predicted_prices, test_data['Close'])
    else:
        mse=metrics.mean_squared_error(df['predicted_prices'][:df.shape[0]-2], df['actual_prices'][:df.shape[0]-2])
        mae=metrics.mean_absolute_error(df['predicted_prices'][:df.shape[0]-2], df['actual_prices'][:df.shape[0]-2])
    #MSE 的值越小，說明預測模型描述實驗資料具有更好的精確度。
    #except:
     #   messages.success(request,get_text+' 此股票代非上市股票')
      #  return redirect('predict:predict')
    return render(request,'predict/predict_pic.html',{'fig':div,'fig2':div2,'Prediction':prediction,'mse':mse,'mae':mae})