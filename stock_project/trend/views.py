...
def trend_stock(request):
    ...
    for i in range(len(data)-1):
        if targets == data[i]['name']:
            stock_number=data[i]['stock']
            ...
            title_text=stock_number+' '+targets
        elif targets ==data[i]['stock']:
...
def trend2(request):
    ...
    with open('test3.json',encoding="utf_8") as f:
            data = json.load(f)
    for i in range(len(data)-1):
        if target == data[i]['name']:
            stock_number=data[i]['stock']
            res = requests.get('https://tw.stock.yahoo.com/_td-stock/api/resource/FinanceChartService.ApacLibraCharts;autoRefresh=1653627795519;symbols=%5B%22'+stock_number+'.TW%22%5D;type=tick?bkt=&device=desktop&ecma=modern&feature=ecmaModern%2CuseVersionSwitch%2CuseNewQuoteTabColor&intl=tw&lang=zh-Hant-TW&partner=none&prid=a3olkn1h90moe&region=TW&site=finance&tz=Asia%2FTaipei&ver=1.2.1295&returnMeta=true')
            title_text=stock_number+' '+target
        elif target ==data[i]['stock']:
            res = requests.get('https://tw.stock.yahoo.com/_td-stock/api/resource/FinanceChartService.ApacLibraCharts;autoRefresh=1653627795519;symbols=%5B%22'+target+'.TW%22%5D;type=tick?bkt=&device=desktop&ecma=modern&feature=ecmaModern%2CuseVersionSwitch%2CuseNewQuoteTabColor&intl=tw&lang=zh-Hant-TW&partner=none&prid=a3olkn1h90moe&region=TW&site=finance&tz=Asia%2FTaipei&ver=1.2.1295&returnMeta=true')
            stock_name=data[i]['name']
            title_text=target+' '+stock_name
    jd = res.json()['data']
    close = jd[0]['chart']['indicators']['quote'][0]['close']
    volume = jd[0]['chart']['indicators']['quote'][0]['volume']
    timestamp = jd[0]['chart']['timestamp']
